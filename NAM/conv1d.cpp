...
void Conv1D::Process(const Eigen::MatrixXf& input, const int num_frames)
{
  ...
  if (this->_is_depthwise)
  {
    ...
    const int channels = this->_channels;
    float* __restrict output_ptr = _output.data();
    ...
    for (size_t k = 0; k < kernel_size; k++)
    {
      ...
      const float* __restrict input_ptr = input_block.data();
      const float* __restrict weight_ptr = this->_depthwise_weight[k].data();
  ...
  else
  {
    ...
    if (kernel_size == 3 && out_ch == 4 && in_ch == 4)
    {
      ...
      const float* __restrict in0_ptr = in0.data();
      const float* __restrict in1_ptr = in1.data();
      const float* __restrict in2_ptr = in2.data();
      float* __restrict output_ptr = _output.data();

      // Get weight pointers for all 3 taps
      const size_t wsize = 16; // 4x4
      const float* __restrict w0 = this->_weight[0].data();
      const float* __restrict w1 = this->_weight[1].data();
      const float* __restrict w2 = this->_weight[2].data();
      ...
      {
        const float b0 = this->_bias(0), b1 = this->_bias(1);
        const float b2 = this->_bias(2), b3 = this->_bias(3);
        for (int f = 0; f < num_frames; f++)
        {
          const int off = f * 4;
          output_ptr[off]     = output_ptr[off] + b0;
          output_ptr[off + 1] = output_ptr[off + 1] + b1;
          output_ptr[off + 2] = output_ptr[off + 2] + b2;
          output_ptr[off + 3] = output_ptr[off + 3] + b3;
        }
      }
    }
    ...
    else if (kernel_size == 6 && out_ch == 3 && in_ch == 3)
    {
      ...
      const float* __restrict in_ptrs[6] = {in0.data(), in1.data(), in2.data(), in3.data(), in4.data(), in5.data()};
      float* __restrict output_ptr = _output.data();
    ...
    else
    {
      ...
      for (size_t k = 0; k < kernel_size; k++)
      {
        ...
        const float* __restrict input_ptr = input_block.data();
        const float* __restrict weight_ptr = this->_weight[k].data();
        float* __restrict output_ptr = _output.data();
        ...
        else if (out_ch == 6 && in_ch == 6)
        {
          ...
          for (int f = 0; f < num_frames; f++)
          {
            const float* __restrict in_col = input_ptr + f * 6;
            float* __restrict out_col = output_ptr + f * 6;
        ...
        else if (out_ch == 8 && in_ch == 8)
        {
          ...
          for (int f = 0; f < num_frames; f++)
          {
            const float* __restrict in_col = input_ptr + f * 8;
            float* __restrict out_col = output_ptr + f * 8;
  ...
  if (this->_bias.size() > 0)
  {
    ...
    const int out_ch = (int)get_out_channels();
    float* __restrict output_ptr = _output.data();
    const float* __restrict bias_ptr = this->_bias.data();
